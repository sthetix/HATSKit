...
def compute_content_hash(user_choices):
    ...
    return hasher.hexdigest()[:7]  # Use first 7 chars for brevity
...
def run_builder():
    ...
    while True:  # Loop to allow restarting the builder
        ...
        if github_pat is None:
            console.print("[dim]A GitHub PAT is optional but recommended to avoid API rate limits.[/]")
            pat_input = questionary.password(
                "Enter GitHub PAT (press Enter to skip, type 'back' to return to main menu):",
                qmark="🔑",
                style=custom_style
            ).ask()
            if pat_input == 'back':
            ...
            if github_pat:
                console.print("[green]GitHub PAT set successfully. It will be used for this session.[/]")
            else:
                console.print("[yellow]No GitHub PAT provided. Proceeding without authentication (rate limits may apply).[/]")
        ...
        # Check if the current selection matches the last build
        ...
        if last_hash == content_hash and last_filename and os.path.exists(os.path.join(base_path, last_filename)):
            console.print(f"[yellow]No updates found. The selected components match the last build: {last_filename}[/]")
            choice = questionary.select(
                "What would you like to do?",
                choices=[
        ...
        console.print(summary_table)
        console.print(f"[dim]Proposed output: {output_filename}[/]")
...
def reset_pat():
    global github_pat
    if github_pat is None:
        console.print("[yellow]No GitHub PAT is currently set.[/]")
    elif questionary.confirm("Are you sure you want to clear the GitHub PAT? You will be prompted to enter a new one next time you use the builder.", style=custom_style).ask():
        github_pat = None
        console.print("[yellow]GitHub PAT has been cleared. You will be prompted to enter a new one next time you use the builder.[/]")
    else:
        console.print("[yellow]GitHub PAT was not cleared.[/]")
    questionary.press_any_key_to_continue("Press any key to return to the main menu...", style=custom_style).ask()
